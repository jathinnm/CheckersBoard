...
void checkers::Square::SetSquareColor(string set_color) {
  color_ = set_color;
}

void checkers::Square::SetGamePiece(GamePiece set_piece) {
  piece_ = set_piece;
}
...
bool checkers::Square::GetContainsGamePiece() const { return contains_game_piece_;}

void checkers::Square::UpdateGamePieceMoves(vec2 &possible_move) {
  piece_.UpdatePossibleMoves(possible_move);
}

bool checkers::Square::GetPieceIsRedColor() const { return piece_.GetIsRedColor();}

const vector<vec2> checkers::Square::GetPiecePossibleMoves() const {return piece_.GetPossibleMoves();}

checkers::GamePiece checkers::Square::GetGamePiece() const {
  return piece_;
}
...
vec2 Square::GetXLim() const { 
  return x_lim_;
}
vec2 Square::GetYLim() const { 
  return y_lim_;
}
